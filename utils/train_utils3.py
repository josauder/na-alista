...
def train_model(m, n, s, k, p, model_fn, noise_fn, epochs, initial_lr, name, model_dir='res/models/',
    ...
    std = size / 6
    mean = (size +1)/2
    sample = np.random.normal(mean, std, size=(int(size**2 * 0.3), 2))
    ...
    train_transform = Compose([RandomAffine((0, 0), scale=(0.8, 0.8)),
                               Grayscale(), RandomAffine((0, 0), translate=(0, 0.1), scale=(0.8, 1.2)),
    ...
    test_transform = Compose(
        [RandomAffine((0, 0), scale=(0.8, 0.8)), Resize(size), CenterCrop(size), Grayscale(), ToTensor()])
